...
public class PersonExcelWriter {
    ...
    private void prepareLineChart(List<Person> personList, XSSFSheet personSheet) {
        ...
        Integer[] yArray = getYIntegerArray(personSheet);
     ...
     * @return
     */
    private Integer[] getYIntegerArray(XSSFSheet personSheet) {
        //get list of supported formulas
        //Collection<String> supportedFunctionNames = WorkbookEvaluator.getSupportedFunctionNames();

        //get list of not supported formulas
        //Collection<String> notSupportedFunctionNames = WorkbookEvaluator.getNotSupportedFunctionNames();


        List<Integer> ageList = new ArrayList<>();

        FormulaEvaluator evaluator = personSheet.getWorkbook().getCreationHelper().createFormulaEvaluator();
        registerFunction();

        int firstRowNumber = 2;
        int lastRowNumger = personSheet.getLastRowNum();

        for (int i=firstRowNumber; i<=lastRowNumger; i++) {
            Cell cell = personSheet.getRow(i).getCell(6);
            int age = (int) evaluator.evaluate(cell).getNumberValue();
            ageList.add(age);
        }

        Integer[] yearArray = new Integer[ageList.size()];
        ageList.toArray(yearArray);
        return yearArray;
    }
    ...
    private void registerFunction() {
        FunctionEval.registerFunction("DATEDIF", new Function() {
            ...
            public ValueEval evaluate(ValueEval[] args, int srcRowIndex, int srcColumnIndex) {
                ...
                try{
                    valueEval1 = OperandResolver.getSingleValue(args[0], srcRowIndex,srcColumnIndex);
                    Date date1 = DateUtil.getJavaDate(OperandResolver.coerceValueToDouble(valueEval1));
                    LocalDate localDate1 = date1.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

                    valueEval2 = OperandResolver.getSingleValue(args[1], srcRowIndex,srcColumnIndex);
                    Date date2 = DateUtil.getJavaDate(OperandResolver.coerceValueToDouble(valueEval2));
                    LocalDate localDate2 = date2.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

                    switch( OperandResolver.coerceValueToString(args[2]).charAt(0)){
                        case 'y':
                            int age =  (int) YEARS.between(localDate1, localDate2);;
                            return new NumberEval(age);
                        default:
                            System.out.println("DATEDIFF: Unknown third parameter " +
                                    OperandResolver.coerceValueToString(args[2]).charAt(0) );
                            return ErrorEval.NUM_ERROR;
                    }

                } catch (EvaluationException e) {
                    System.out.println("DateDiff Error --> " + args + " " + srcRowIndex + "  " +
                            srcColumnIndex);
                    e.printStackTrace();
                }
                return ErrorEval.NA;
